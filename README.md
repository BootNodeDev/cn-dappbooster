...
flowchart TD
  ...
  wallet["carpincho-wallet<br/>Vault + signer<br/>http://localhost:3013 in tools mode"]
  tools["canton-barebones/wallet-gateway-tools<br/>Facade + dev helpers<br/>http://localhost:3011"]
  ...
  wallet -->|"one configured RPC URL<br/>/rpc"| tools
  tools -->|"proxy standard wallet-gateway routes"| wg
  tools -->|"Scan API / token metadata"| scan
  tools -->|"configured Canton auth<br/>dev helpers"| au
...
profiles, then starts the wallet-gateway-tools facade. It does not start Keycloak or OIDC.
...
Splice, wallet-gateway, and wallet-gateway-tools share the `canton-barebones` Docker Compose project,
...
| `env/.env.wallet-gateway-tools` | tools endpoints and auth         |
...
cp canton-barebones/env/examples/.env.wallet-gateway-tools.example canton-barebones/env/.env.wallet-gateway-tools
```
...
| ------------------------------------ | ----------------------------------- | --------------------- |
| `AUTH_MODE=self-signed`              | local JWT signed with `AUTH_SECRET` | wallet-gateway-tools |
| `AUTH_MODE=oauth-client-credentials` | OAuth client credentials flow       | wallet-gateway-tools |
| `AUTH_MODE=static-token`             | externally obtained bearer token    | wallet-gateway-tools |
...
Tools mode also exposes wallet-gateway-tools on `http://localhost:3011`.
Carpincho should point at `http://localhost:3011/rpc` when you want the
development helper RPCs. Tools mode owns the Canton, Scan, validator, and
...
project moves, run it on another dev port when wallet-gateway-tools is up:
...
| ------------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------ |
| wallet-gateway            | Official wallet-gateway                                    | `http://localhost:3010`                                                                            | direct wallet-gateway clients/tools |
| wallet-gateway-tools     | Public facade for official wallet-gateway plus dev helpers | `http://localhost:3011`                                                                            | Carpincho via `/rpc`, tools          |
| Carpincho wallet          | Browser wallet UI/provider                                 | `http://localhost:3013` during tools mode                                                         | user/dApp                            |
...
| app-user Admin API        | gRPC Admin API                                             | `grpc://localhost:2902`                                                                            | wallet-gateway-tools/tools          |
| app-user Validator API    | Splice validator readiness/API                             | `http://localhost:2903`                                                                            | health/tools                         |
| app-user JSON API         | JSON Ledger API                                            | `http://localhost:2975`                                                                            | wallet-gateway-tools/tools          |
| app-user Validator proxy  | wallet-sdk validator route                                 | `http://localhost:2000/api/validator`                                                              | wallet-gateway-tools/tools          |
...
| Scan API                  | Splice indexed API                                         | `http://scan.localhost:4000/api/scan`                                                              | wallet-gateway-tools/tools          |
| Amulet Registry           | token metadata via scan proxy                              | `http://localhost:2000/api/validator/v0/scan-proxy`                                                | wallet-gateway-tools/tools          |
