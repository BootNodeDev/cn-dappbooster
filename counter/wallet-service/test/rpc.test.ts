...
describe('rpc dispatcher', () => {
  it('returns -32600 when jsonrpc is not "2.0"', async () => {
    ...
    const res = (await rpc.handle({ jsonrpc: '1.0' as never, id: 1, method: 'status' })) as JsonRpcResponse
  ...
  it('returns -32600 when method is not a string', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: 1, method: 42 as never })) as JsonRpcResponse
    assert.ok('error' in res)
    assert.equal(res.error.code, -32600)
  })
  ...
  it('returns -32601 for unknown methods', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: 1, method: 'doesNotExist' })) as JsonRpcResponse
    assert.ok('error' in res)
  ...
  it('returns -32004 for prepareExecute (carpincho signs)', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: 1, method: 'prepareExecute' })) as JsonRpcResponse
    assert.ok('error' in res)
  ...
  it('listAccounts returns []', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: 1, method: 'listAccounts' })) as JsonRpcResponse
  ...
  it('getActiveNetwork returns the configured network', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: 1, method: 'getActiveNetwork' })) as JsonRpcResponse
    assert.ok('result' in res)
...
describe('JSON-RPC notifications', () => {
  ...
  it('returns a response for id: null (explicit, not a notification)', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({ jsonrpc: '2.0', id: null, method: 'disconnect' })) as JsonRpcResponse
...
describe('invalid params mapping', () => {
  ...
  it('returns -32602 when ledgerApi is missing resource', async () => {
    const rpc = createRpc(baseConfig())
    const res = (await rpc.handle({
      jsonrpc: '2.0', id: 1, method: 'ledgerApi',
