...
`canton-barebones/wallet-gateway-tools` aligns its wire format with this spec but does not
...
gh api repos/canton-network/wallet/contents/api-specs/openrpc-dapp-api.json --jq '.content' \
  | base64 -d > canton-barebones/wallet-gateway-tools/api-specs/openrpc-dapp-api.json
```
