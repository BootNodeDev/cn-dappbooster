...
| Styling | Tailwind CSS v4 | `@tailwindcss/vite` plugin; utility classes inline in JSX; `src/index.css` declares semantic CSS variables on `:root` / `[data-theme='dark']`, rebinds the `dark:` variant via `@custom-variant`, exposes tokens to Tailwind through `@theme inline`, and holds `@layer base` resets plus named keyframes (`fade-in`, `slide-down-and-fade`, `slide-up-and-fade`, `sheet-up`, `sheet-slide-right`, `slide-in-right`, `slide-in-left`, `soft-pulse`, `drift`). A low-opacity paper-grain SVG noise overlay is applied via `body::before` |
| Theming | Manual light/dark toggle (Riverbank Organic palette) | `src/theme/ThemeProvider.tsx` owns the theme state, persists to `localStorage`, defaults to `prefers-color-scheme`, and writes `data-theme` on `<html>` after mount. Use semantic colour utilities (`bg-surface`, `text-foreground`, `border-border`, `bg-primary-soft`, `bg-scrim`, etc.) so styles flip automatically. Iris/lupine purples (`primary` / `accent`) anchor the palette; warm parchment + taupe neutrals replace cold greys; `--color-success` is green (used for the connected-state indicator) against the otherwise warm palette; `--color-scrim` is the theme-aware overlay tint used by modal/sheet backdrops |
| Fonts | Self-hosted via `@fontsource-variable/fraunces`, `@fontsource-variable/manrope`, `@fontsource-variable/roboto-mono` | Imported once in `src/main.tsx`; works offline in the extension popup. Use `font-display` (Fraunces) for hero wordmarks, view-level headings, section markers; `font-sans` (Manrope) for UI chrome / body / labels / buttons; `font-mono` (Roboto Mono) for party IDs, hashes, RPC URLs, JSON payloads, status eyebrows |
...
| Linting | Biome | Enforced via root-level pre-commit hook (lint-staged dispatches `biome check` against `carpincho-wallet/src/**`); run `npm run lint` manually |
...
- Dev server runs at `http://localhost:3011`. From the monorepo root, the standard entry point is `npm run wallet:dev`.
