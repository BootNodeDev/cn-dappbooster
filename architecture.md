...
├── canton-base/                   Local Canton participant + Postgres + wallet-service
...
- `carpincho-wallet` holds all signing keys (PBKDF2 + AES-GCM vault, Ed25519). It never talks to Canton directly; it goes through `canton-base/wallet-service`.
- `canton-base/wallet-service` is the only component holding the Canton bearer token. It self-mints the token at boot, then validates and forwards JSON-RPC calls onto the participant's JSON API.
...
| `CANTON_BACKEND_TOKEN` | `canton-base/wallet-service` runtime env (compose) | Self-minted at boot from `CANTON_AUTH_AUDIENCE` / `CANTON_AUTH_SECRET` / `CANTON_BACKEND_USER_ID`. Set explicitly (e.g. via the compose env override) to bypass self-mint. Mint ad-hoc with `npm run canton:token`. |
...
| `npm --prefix canton-base/wallet-service run dev` | Host-side dev mode for the wallet-service on `:3010` (tsx watch). The container-side service is started by `canton:up`. |
| `npm run wallet-service:health` | Hit the wallet-service health endpoint at `:3010` |
| `npm --prefix canton-base run wallet-service:logs` | Tail the wallet-service container's logs |
...
- [`canton-base/wallet-service/README.md`](canton-base/wallet-service/README.md) — JSON-RPC bridge
